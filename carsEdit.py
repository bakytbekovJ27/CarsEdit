...
# Function to modify existing car information
def modify_car_info():
    ...
    try:
        folder_choice = int(input("\nEnter the number of the brand you want to modify: ")) - 1
        if 0 <= folder_choice < len(folders):
            selected_folder = folders[folder_choice]
            files = [f for f in os.listdir(selected_folder) if f.endswith('.txt')]
            
            if not files:
                print(f"No cars available in '{selected_folder}' brand.")
                return
            ...
            file_choice = int(input("\nEnter the number of the car you want to modify: ")) - 1
            if 0 <= file_choice < len(files):
                selected_file = files[file_choice]
                ...
                car_model = input("Enter the new Car Model: ")
                car_year = input("Enter the new Year of Manufacture: ")
                car_price = input("Enter the new Price of the Car: ")

                new_car_info = f"Car ID: {selected_file[:-4]}\nCar Model: {car_model}\nYear of Manufacture: {car_year}\nPrice: {car_price}\n"
                
                with open(file_path, "w") as f:
                    f.write(new_car_info)

                print(f"\nCar information updated in '{file_path}'\n")
            else:
                print("Invalid choice.")
        else:
            print("Invalid choice.")
...
def delete_car_info():
    ...
    try:
        folder_choice = int(input("\nEnter the number of the brand you want to delete from: ")) - 1
        if 0 <= folder_choice < len(folders):
            selected_folder = folders[folder_choice]
            files = [f for f in os.listdir(selected_folder) if f.endswith('.txt')]
            ...
            if 0 <= file_choice < len(files):
                ...
                # Confirm deletion
                confirm = input(f"Are you sure you want to delete '{selected_file}'? (y/n): ")
                if confirm.lower() == 'y':
                    os.remove(file_path)
...
# Main function to show the menu with added modify and delete options
def main():
    ...
    while True:
        ...
        print("3. Modify car information")
